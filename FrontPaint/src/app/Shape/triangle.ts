...
export class triangle implements Shape {
  ...
  startDraw(event: MouseEvent) {
    ...
    if (this.counter === 0) {
      ...
      this.triangle.setAttribute("id", this.id);
      this.triangle.setAttribute("stroke-width", "3");
      this.renderer.setAttribute(this.triangle, "stroke", this.stroke);
      this.renderer.setAttribute(this.triangle, "fill", this.fill);
      this.counter++;
      return;
    }
  ...
  remove(array: Shape[] = []) {
    this.triangle = document.getElementById(this.id.toString());
...
setAttributes(attributes: string){
  ...
  this.x3 = parseInt(data[5]);
  this.y3 = parseInt(data[6]);
  this.id = parseInt(data[7])
  this.fill = data[8];  
  this.stroke =data[9]; 
